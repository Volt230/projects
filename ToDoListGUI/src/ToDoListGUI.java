...
public class ToDoListGUI extends JFrame {
    ...
    private void openEditTaskDialog() {
        ...
        // Populate Edit Table with Current Task Data
        for (ToDoListLogic.Task task : logic.getTasks()) {
            boolean isDone = task.getStatus().equals("✔");
            boolean isNotDone = task.getStatus().equals("✖");
            // If the status is unknown or anything else, both checkboxes are unchecked.
            editTableModel.addRow(new Object[]{task.getName(), task.getPriority(), isDone, isNotDone});
        }
        ...
        saveChangesButton.addActionListener(e -> {
            for (int i = 0; i < editTableModel.getRowCount(); i++) {
                boolean isDone = (boolean) editTableModel.getValueAt(i, 2); // Done checkbox
                boolean isNotDone = (boolean) editTableModel.getValueAt(i, 3); // Not Done checkbox
